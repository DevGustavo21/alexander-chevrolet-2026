...
(() => {
  // Leading dot removed so Next.js' public/ static handler serves it
  // (dotfiles are denied by default in production).
  ...
  const stylesheet =
    ...
    // touch-action:auto on the static image so vertical page scroll passes
    // through on mobile — the slot is read-only on the live site (reframe
    // requires window.omelette) and the reframe .spill layer below keeps
    // its own touch-action:none for pan/resize gestures when actually in
    // edit mode.
